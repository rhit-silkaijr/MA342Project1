% Returns the length and design coefficients
% of a needed airfoil wing for a given
% two-dimensional wing profile
% P: the points of the 2-D profile. Must be in a n x 2 format,
% where each row represents the (x, z) coordinate of every point
% the last point will represent the wake effect and the 
% second-to-last point will be a repeat of the first
%
% m: the mass of the aircraft in kg
%
% v: the free-stream velocity the airfoil will be under, in
...
function [gamma] = airfoil_wing(P, v, plt)
    ...
    % wing profile
    disp = (abs(max(P(:, 2))) + abs(min(P(:, 2))))*0.01;
    P(end - 1, 2) = P(end - 1, 2) + sign(P(end - 1, 2))*disp;
    adj = 2;
    ...
    % Now we can find our influence coefficients into a matrix. The value
    % is dependent on its location, with the diagonal elements receiving
    % different results compared to the others
    ...
    % Now to compute our A matrix, which is simply each normal vector
    % for each influence coefficient
    ...
    % Apply freestream velocities on each panel
    b = v * n_end * [1; 0];
    
    % doublet strengths found by inversing the A matrix on our
    % b vector
    mu = A\b;
    ...
    if plt
        ...
        axis equal;
        plot(P(:, 1), P(:, 2), 'bo-');
        plot(mp_panels(:, 1), mp_panels(:, 2), 'rx');
        title('Airfoil Diagram');
        xlabel('x (m)');
        ylabel('z (m)');
        hold off;
        legend('Panel Points', 'Panel Midpoints');
    end
end